...
function startServer(mainWindow: BrowserWindow, discovery: DiscoveryNetwork) {
	...
	ipcMain.on("launchMissileRequest", () => {
		console.log(
			"ServerQueries.ts: received launch missile request from renderer",
		);
		launchQuery();
	});
	...
	server.post("/droneLoc", (req, res) => {
		res.send("ok");
		console.log(
			"ServerQueries.ts: received drone location ping from Blue GUI",
		);
		const data = req.body;
	...
	server.post("/missileLoc", (req, res) => {
		res.send("ok");
		console.log(
			"ServerQueries.ts: received missile location ping from Blue GUI",
		);
		const data = req.body;
		console.log(data);
...
async function launchQuery() {
	...
	try {
		...
		if (networkURL != "") {
			targetURL = `${networkURL}pingMissileLaunch`;
		}
...
async function launcherLocQuery(x: number, y: number) {
	...
	try {
		let targetURL = localhost_url;
		if (networkURL != "") {
			targetURL = `${networkURL}pingLauncherLoc`;
		}
