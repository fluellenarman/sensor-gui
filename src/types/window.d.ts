...
declare global {
	interface Window {
		...
		electronAPI: {
			...
			onUpdateDevices: (callback: (devices: DeviceConnections) => unknown) => void
			// try to connect or disconnect a device
			trySetConnection: (path: string, connected: boolean) => Promise<true | string>
