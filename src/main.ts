...
import { startServer, sendTestQuery } from "./util/ServerQueries";
import { DiscoveryNetwork } from "./util/network/discovery";
...
const createWindow = () => {
	...
	ipcMain.handle(
		"save-cage",
		async (event: IpcMainInvokeEvent, cage: CageData) => {
			try {
				writeFileSync(
					path.join(
						app.getPath("appData"),
						"god-sensor-cage-config.json",
					),
					JSON.stringify(cage),
					{
						encoding: "utf8",
